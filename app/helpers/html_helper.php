...
function btn_action($data = []) {
	...
	$html = '<div class="btn-action-group" style="position: relative; display: -ms-inline-flexbox; display: inline-flex; vertical-align: middle;">';
	$attr = '';
	foreach ($data as $key => $val) {
		if ($key == 'edit') {
			...
			$html .= '<a href="'.$data[$key]['url'].'" class="btn btn-warning btn-xs mr-1">
						<span class="btn-label-icon"><i class="fa fa-eye pr-1"></i></span> View 
					</a>';
		}
		else if ($key == 'claim') {
			$html .= '<a href="'.$data[$key]['url'].'" class="btn btn-success btn-xs mr-1">
						<span class="btn-label-icon"><i class="fa fa-check pr-1"></i></span> Claim 
					</a>';
		}
