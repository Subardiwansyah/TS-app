...
switch ($_GET['action']) 
{
	...
	case 'index':
		...
		if (!empty($_POST['delete'])) 
		{
			...
			$result = $db->delete('brand', ['id_brand' => $_POST['id']]);
			// $result = true;
			if ($result) {
	...
	case 'add':
		...
		if (isset($_POST['submit'])) 
		{
			...
			$validation->setRules('nama_brand', 'Nama Brand', 'trim|required');
		...
		if (isset($_POST['submit'])) 
		{
			...
			if ($form_errors) {
				...
				if (!$error) {
					$data_db['updated_date'] = date('Y-m-d H:i:s');
					$data_db['updated_by'] = $user['nama'];
					$query = $db->update('brand', $data_db, 'id_brand = ' . $_POST['id']);
					if ($query) {
						$data['msg']['message'] = 'Data berhasil disimpan';
						header('location:'.module_url());
					} else {
	...
	case 'getDataDT':
		...
		foreach ($data_table['content'] as $key => &$val) 
		{
			...
			$val['ignore_search_action'] = btn_action([
									'edit' => ['url' => BASE_URL . $current_module['nama_module'] . '/edit?id='. $val['id_brand']]
									, 'delete' => ['url' => ''
												, 'id' =>  $val['id_brand']
												, 'delete-title' => 'Hapus data brand: <strong>'.$val['nama_brand'].'</strong> ?'
											]
									,'view' => ['url' => BASE_URL . $current_module['nama_module'] . '/view?id='. $val['id_brand']]
							]);
...
function getListData() {
	...
	// Query Total
	$sql = 'SELECT COUNT(*) AS jml_data FROM brand ';
	$query = $db->query($sql)->getRowArray();
	$total_data = $query['jml_data'];
	
	// Query Filtered
	$sql = 'SELECT COUNT(*) AS jml_data FROM brand '.$where;
	$query = $db->query($sql)->getRowArray();
	$total_filtered = $query['jml_data'];
	
	// Query Data
	$sql = 'SELECT * FROM brand ' . $where  . $order;
