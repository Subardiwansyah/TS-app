...
switch ($_GET['action']) 
{
	...
	case 'index':
		...
		if (!empty($_POST['delete'])) 
		{
			...
			$result = $db->delete('faq', ['id_faq' => $_POST['id']]);
			// $result = true;
			if ($result) {
	...
	case 'add':
		...
		if (isset($_POST['submit'])) 
		{
			...
			$validation->setRules('question', 'Question', 'trim|required|unique[faq]');
			$validation->setRules('answer', 'Answer', 'trim|required');
	...
	case 'edit':
		...
		if (empty($_REQUEST['id'])) {
			...
			$result['msg']['message'] = 'Data faq yang ingin diedit tidak ditemukan';
		} else {
		...
		if (isset($_POST['submit'])) 
		{
			...
			$validation = new FormValidation();
			$unique = false;
			if ($_POST['question'] != $_POST['question_old']) {
				$unique = true;
			}
			$fields = ['question', 'answer','enabled'];
			...
			if ($form_errors) {
				...
				$query = $db->update('faq', $data_db, 'id_faq = ' . $_POST['id']);
	...
	case 'getDataDT':
		...
		foreach ($data_table['content'] as $key => &$val) 
		{
			...
			$val['ignore_search_action'] = btn_action([
									'edit' => ['url' => BASE_URL . $current_module['nama_module'] . '/edit?id='. $val['id_faq']]
									, 'delete' => ['url' => ''
												, 'id' =>  $val['id_faq']
												, 'delete-title' => 'Hapus data faq: <strong>'.$val['question'].'</strong> ?'
											]
									,'view' => ['url' => BASE_URL . $current_module['nama_module'] . '/view?id='. $val['id_faq']]
							]);
...
function getListData() {
	...
	// Query Total
	$sql = 'SELECT COUNT(*) AS jml_data FROM faq ';
	$query = $db->query($sql)->getRowArray();
	$total_data = $query['jml_data'];
	
	// Query Filtered
	$sql = 'SELECT COUNT(*) AS jml_data FROM faq '. $where;
	$query = $db->query($sql)->getRowArray();
	$total_filtered = $query['jml_data'];
	
	// Query Data
	$sql = 'SELECT * FROM faq ' . $where  . $order;
...
function validate_form() {
	...
	require_once('app/libraries/FormValidation.php');
	$validation = new FormValidation();
	$validation->setRules('question', 'Question', 'trim|required');
	$validation->setRules('answer', 'Answer', 'trim|required');
