...
switch ($_GET['action']) 
{
	...
	case 'index':
		...
		$data['title'] = 'Data Dealer';
		$sql = 'SELECT * FROM user LEFT JOIN role USING(id_role)' . where_own()." and id_role = '5'";
	...
	case 'getDataDT':
		...
		foreach ($data_table['content'] as $key => &$val) 
		{
			...
			$val['ignore_search_action'] = btn_action([
									...
									, 'delete' => ['url' => ''
												, 'id' =>  $val['id_user']
												, 'delete-title' => 'Hapus data dealer: <strong>'.$val['nama'].'</strong> ?'
											]
...
function getListData() {
	...
	// Query Total
	$sql = 'SELECT COUNT(*) AS jml_data FROM user LEFT JOIN role USING(id_role) ' . where_own() .' AND id_role=5';
	$query = $db->query($sql)->getRowArray();
	$total_data = $query['jml_data'];
	
	// Query Filtered
	$sql = 'SELECT COUNT(*) AS jml_data FROM user LEFT JOIN role USING(id_role) ' . $where.' AND id_role=5';
	$query = $db->query($sql)->getRowArray();
	$total_filtered = $query['jml_data'];
	
	// Query Data
	$sql = 'SELECT * FROM user LEFT JOIN role USING(id_role) ' . $where  .' AND id_role=5 '. $order;
