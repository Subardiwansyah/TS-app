...
switch ($_GET['action']) 
{
	...
	case 'index':
		...
		$sql = 'SELECT * FROM user LEFT JOIN role USING(id_role)' . where_own()." and id_role = '6'";
	...
	case 'add':
		...
		if (isset($_POST['submit'])) 
		{
			...
			if (!$error) {
				...
				$data['title'] = 'Edit Data Principal';
...
function getListData() {
	...
	// Query Total
	$sql = 'SELECT COUNT(*) AS jml_data FROM user LEFT JOIN role USING(id_role) ' . where_own() .' AND id_role=6';
	$query = $db->query($sql)->getRowArray();
	$total_data = $query['jml_data'];
	
	// Query Filtered
	$sql = 'SELECT COUNT(*) AS jml_data FROM user LEFT JOIN role USING(id_role) ' . $where.' AND id_role=6';
	$query = $db->query($sql)->getRowArray();
	$total_filtered = $query['jml_data'];
	
	// Query Data
	$sql = 'SELECT * FROM user LEFT JOIN role USING(id_role) ' . $where  .' AND id_role=6 '. $order;
