			...
			if (!$error) {
				...
				foreach ($fields as $field) {
					...
					if(isset($_POST['id_role'])){
						$data_db['no_user'] = no_user($_POST['id_role']);
					}
				}
...
function getListData() {
	...
	// Query Total
	$sql = 'SELECT COUNT(*) AS jml_data FROM user LEFT JOIN role USING(id_role) ' . where_own().' AND id_role not like "4"';
	$query = $db->query($sql)->getRowArray();
	$total_data = $query['jml_data'];
	
	// Query Filtered
	$sql = 'SELECT COUNT(*) AS jml_data FROM user LEFT JOIN role USING(id_role) ' . $where.' AND id_role not like "4"';
	$query = $db->query($sql)->getRowArray();
	$total_filtered = $query['jml_data'];
	
	// Query Data
	$sql = 'SELECT * FROM user LEFT JOIN role USING(id_role) ' . $where.' AND id_role not like "4"' . $order;
