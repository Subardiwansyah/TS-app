...
function csrf_verify() 
{
	...
	if (!empty($_POST))
	{
		...
		if (empty($_COOKIE[$csrf_token['name']]) || empty($_POST[$csrf_token['name']])) {
			...
			$csrf_token['validation']['message'] = 'Error, Token Not Found. Please enable cookies in your browser.';
			$error = true;
		}
		...
		if ( !empty($_POST[ $csrf_token['name'] ]) && !empty($_COOKIE[ $csrf_token['name']] ) ) {
			if ($_COOKIE[$csrf_token['name']] != @$_POST[$csrf_token['name']]) {
				...
				$csrf_token['validation']['message'] = 'Error, Token Missmatch. Please try again.';
				$error = true;
			}
		}
